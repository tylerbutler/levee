/**
 * FocusTracker - tracks the focus state of all connected sessions using
 * Fluid Framework presence features.
 */
 ...
 *
 * - focusChanged: Emitted any time the FocusTracker detects a change in focus
 *   in any client, local or remote.
 */
export type IFocusTrackerEvents = {
	focusChanged: [focusState: IFocusState];
};

/**
 * The FocusTracker class tracks the focus state of all connected sessions
 * using the Fluid Framework presence features. Focus state is tracked
 * automatically by the class instance. As the focus state of connected
 * sessions change, the FocusTracker emits a "focusChanged" event.
 */
export class FocusTracker extends TypedEventEmitter<IFocusTrackerEvents> {
	...
	public constructor(
		...
		/**
		 * A states workspace that the FocusTracker will use to share focus
		 * states with other session clients.
		 */
		public readonly statesWorkspace: StatesWorkspace<StatesWorkspaceSchema>,
	) {
		super();

		// Create a Latest state object to track the focus state.
		// The value is initialized with current focus state of the window.
		statesWorkspace.add(
			"focus",
		...
		this.focus = statesWorkspace.states.focus as LatestRaw<IFocusState>;

		// When the focus state is updated, the FocusTracker should emit the focusChanged event.
		this.focus.events.on("remoteUpdated", () => {
		...
		window.addEventListener("blur", () => {
			this.focus.local = { hasFocus: false };
