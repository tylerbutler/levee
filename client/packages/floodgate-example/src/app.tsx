/**
 * Standalone Floodgate DiceRoller application.
 *
...
import { createDiceSession, type DiceSession } from "./session.js";
...
interface AppState {
	...
	diceMap?: DiceSession["diceMap"];
	documentId?: string;
	error?: string;
}

/** Main Floodgate DiceRoller application component. */
function App(): React.ReactElement | null {
	const [state, setState] = useState<AppState>({ status: "loading" });
	const sessionRef = useRef<DiceSession | undefined>(undefined);
	...
	useEffect(() => {
		...
		async function initialize(): Promise<void> {
			try {
				...
				const session = await createDiceSession(floodgateClient, {
	...
	return (
		<div>
			<DiceRollerView diceMap={state.diceMap} />
