...
let private makeClaims tenantId documentId scopes exp : TokenClaims =
    { DocumentId = documentId
      Scopes = scopes
      TenantId = tenantId
      User = { Id = "test-user"; Properties = Map.empty }
      IssuedAt = 1000L
      Expiration = exp
      Version = "1.0"
      Jti = None }
...
let signetTests =
    testList "Signet" [
        testList "claim validation" [
            testCase "validate expiration valid" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead ] 2000L
                Expect.isOk (validateExpiration claims 1500L) "not expired"

            testCase "validate expiration expired" <| fun () ->
            ...
            testCase "validate scope present" <| fun () ->
            ...
            testCase "validate scope missing" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead ] 2000L

                match validateScope claims DocWrite with
                | Error(MissingScope(required, _)) -> Expect.equal required DocWrite "required"
                | other -> failtest $"expected MissingScope, got {other}"

            testCase "has scope helpers" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead; DocWrite ] 2000L
            ...
            testCase "validate connection claims ok" <| fun () ->
                let claims = makeClaims "my-tenant" "my-doc" [ DocRead; DocWrite ] 2000L
                Expect.isOk (validateConnectionClaims claims "my-tenant" "my-doc" 1500L) "ok"

            testCase "validate connection claims expired first" <| fun () ->
            ...
            testCase "validate connection claims tenant mismatch" <| fun () ->
            ...
            testCase "validate read access" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead ] 2000L
                Expect.isOk (validateReadAccess claims "tenant" "doc" 1500L) "ok"

            testCase "validate read access missing scope" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocWrite ] 2000L

                match validateReadAccess claims "tenant" "doc" 1500L with
                | Error(MissingScope(required, _)) -> Expect.equal required DocRead "required"
                | other -> failtest $"expected MissingScope, got {other}"

            testCase "validate write access" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead; DocWrite ] 2000L
                Expect.isOk (validateWriteAccess claims "tenant" "doc" 1500L) "ok"

            testCase "validate write access missing write scope" <| fun () ->
                let claims = makeClaims "tenant" "doc" [ DocRead ] 2000L
        ...
        testList "scope conversions" [
            testCase "scope string roundtrip" <| fun () ->
                for scope in [ DocRead; DocWrite; SummaryRead; SummaryWrite ] do
                    Expect.equal (scopeFromString (scopeToString scope)) (Some scope) "roundtrip"

            testCase "scopes from strings drops unknown" <| fun () ->
                Expect.equal
        ...
        testList "crypto" [
            testCase "mint and verify signature roundtrip" <| fun () ->
                let token =
                    mintToken "tenant-1" "doc-1" [ DocRead; DocWrite ] "user-1" "secret" 1000L 3600L "test-jti"
            ...
            testCase "verify rejects wrong secret" <| fun () ->
            ...
            testCase "verify rejects empty secret" <| fun () ->
                let token = mintToken "t" "d" [ DocRead ] "u" "right" 1000L 3600L "j"
                Expect.equal (verifySignature token "") (Error BadSignature) "rejected"

            testCase "verify rejects malformed token" <| fun () ->
                Expect.equal (verifySignature "not-a-jwt" "secret") (Error BadFormat) "rejected"

            testCase "verify rejects non-HS256 alg" <| fun () ->
                ...
                Expect.equal (verifySignature $"{header}.e30.sig" "secret") (Error BadFormat) "alg none rejected"

            testCase "extract token parses bearer and basic schemes" <| fun () ->
                Expect.equal (extractToken "Bearer abc.def.ghi") (Ok "abc.def.ghi") "bearer"
                Expect.equal (extractToken "Basic abc.def.ghi") (Ok "abc.def.ghi") "basic jwt"
                Expect.equal (extractToken "Nonsense") (Error BadFormat) "rejected"

            testCase "extract token decodes basic user:jwt" <| fun () ->
                let encoded =
                    System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes "user:abc.def.ghi")

                Expect.equal (extractToken $"Basic {encoded}") (Ok "abc.def.ghi") "decoded"

            testCase "validate write access end to end" <| fun () ->
                let token =
                    mintToken "tenant-1" "doc-1" [ DocRead; DocWrite ] "user-1" "secret" 1000L 3600L "j"

                match verifySignature token "secret" with
                | Ok claims ->
                    Expect.isOk (validateWriteAccess claims "tenant-1" "doc-1" 2000L) "valid"
                    Expect.isError (validateWriteAccess claims "other" "doc-1" 2000L) "wrong tenant"
                | Error e -> failtest $"expected Ok, got {e}"
        ]
    ]
