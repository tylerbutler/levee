...
let jsonTests =
    testList "Json" [
        ...
        testCase "non-ASCII is raw UTF-8, not escaped" <| fun () ->
            let json = JObj [ "name", JStr "Ünïcödé <&>" ]
            Expect.equal (Json.toString json) "{\"name\":\"Ünïcödé <&>\"}" "matches Erlang json:encode"

        testCase "raw splice is verbatim" <| fun () ->
        ...
        testCase "output is never indented" <| fun () ->
