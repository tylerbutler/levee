...
module DocumentProtocol =
    ...
    let clientJoinData (clientId: string) (clientJson: string) : string =
        JObj [ "clientId", JStr clientId; "detail", JRaw(System.ReadOnlyMemory(System.Text.Encoding.UTF8.GetBytes clientJson)) ]
        |> Json.toString
                ...
                [ "type", JStr "join"
                  ...
                  JObj
                      [ "clientId", JStr clientId
                        "client", JRaw(System.ReadOnlyMemory(System.Text.Encoding.UTF8.GetBytes clientJson)) ] ]
    ...
    let systemMessage (kind: string) (data: string) (sn: int64) (msn: int64) (timestamp: int64) : string =
        JObj
            [ "clientId", JNull
    ...
    let opsArrayJson (ops: System.Collections.Generic.KeyValuePair<int64, string>[]) : byte[] =
        Json.toUtf8 (JArr(ops |> Array.map (fun kv -> storedMessageJson (kv.Key, kv.Value)) |> List.ofArray))
    ...
    let sequencedOpJson (clientId: string) (op: SubmittedOp) (sn: int64) (msn: int64) (timestamp: int64) : string =
        let fields =
            [ "clientId", JStr clientId
              "sequenceNumber", JInt sn
              "minimumSequenceNumber", JInt msn
    ...
    let private nackJson (op: SubmittedOp | null) (sn: int64) (code: int) (message: string) : Json =
        ...
        JObj
            [ "operation", operationJson
              "sequenceNumber", JInt sn
              "content",
              JObj [ "code", JInt(int64 code); "type", JStr "BadRequestError"; "message", JStr message ] ]
    ...
    let nackListJson (nacks: (SubmittedOp | null * int64 * int * string)[]) : byte[] =
        Json.toUtf8 (JArr(nacks |> Array.map (fun (op, sn, code, message) -> nackJson op sn code message) |> List.ofArray))

    // ── Summary wire shapes ─────────────────────────────────────────────────

    let summaryAckJson (handle: string) (summarySn: int64) (msn: int64) (timestamp: int64) : string =
        SessionLogic.buildSummaryAck handle summarySn msn timestamp |> JObj |> Json.toString

    let summaryNackJson (summarySn: int64) (responseSn: int64) (msn: int64) (reason: string) (timestamp: int64) : string =
        JObj
            [ "clientId", JNull
    ...
    let parseSummarizeContents (contentsJson: string) : SummarizeParse =
        ...
        | Some doc ->
            ...
            if el.ValueKind <> JsonValueKind.Object then
                ...
                let fields =
                    el.EnumerateObject() |> Seq.map (fun p -> p.Name, Dyn.toJson p.Value) |> Map.ofSeq
    ...
    let summaryCommitBody (handle: string) (parents: string[]) (message: string) (nowSeconds: int64) : string =
        let author =
            JObj
                [ "name", JStr "Floodgate"
    ...
    let connectedResponse
        ...
        let rawString (s: string) = raw (System.Text.Encoding.UTF8.GetBytes s)
        ...
        JObj
            [ "claims", claimsJson claims
              ...
              JArr(
                  roster
                  |> Array.map (fun kv -> JObj [ "clientId", JStr kv.Key; "client", rawString kv.Value ])
                  |> List.ofArray
              )
              "initialMessages",
              JArr(initialOps |> Array.map (fun kv -> storedMessageJson (kv.Key, kv.Value)) |> List.ofArray)
    ...
    let private toParsed (s: Signals.NormalizedSignal) : ParsedSignal =
        { ContentJson = Json.toString (Json.canonicalize s.Content)
          ...
          TargetedClients = s.TargetedClients |> Option.map Array.ofList |> Option.defaultValue [||]
                        ...
                        let allComplete =
                            ...
                            && List.forall2
                                (fun (l: string list option) (e: JsonElement) ->
                                    ...
                                    | Some strings, Some raw -> List.length strings = List.length raw
                                    | _ -> false)
                                lists
                                entries
    ...
    let signalMessagePayload (clientId: string) (contentJson: string) : byte[] =
        Json.toUtf8 (
            JObj
                [ "clientId", JStr clientId
                  "content", JRaw(System.ReadOnlyMemory(System.Text.Encoding.UTF8.GetBytes contentJson)) ]
        )
    ...
    let signalRecipients (senderClientId: string) (signal: ParsedSignal) (allClientIds: string[]) : string[] =
        SessionLogic.determineSignalRecipients
            senderClientId
            (if signal.HasTargetedClients then Some(List.ofArray signal.TargetedClients) else None)
            (if signal.HasIgnoredClients then Some(List.ofArray signal.IgnoredClients) else None)
            (if signal.HasTargetClientId then Some signal.TargetClientId else None)
