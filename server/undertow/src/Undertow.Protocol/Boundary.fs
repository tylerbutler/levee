...
module AuthBoundary =
    ...
    let private outcome (result: Result<'a, Auth.AuthError>) : AuthOutcome =
        ...
        | Error e -> { Ok = false; ErrorMessage = Auth.errorMessage e }

    let private failWith e : AuthOutcome =
        { Ok = false; ErrorMessage = Auth.errorMessage e }
    ...
    let tenantWrite (configuredTenant: string, tenant: string, hasHeader: bool, authorization: string, secret: string, now: int64) =
        gate configuredTenant tenant hasHeader
            (fun a -> Auth.verifyTenantWriteAuthorization a secret tenant now)
            authorization

    let write (configuredTenant: string, tenant: string, doc: string, hasHeader: bool, authorization: string, secret: string, now: int64) =
        gate configuredTenant tenant hasHeader
            (fun a -> Auth.verifyWriteAuthorization a secret tenant doc now)
            authorization

    let read (configuredTenant: string, tenant: string, doc: string, hasHeader: bool, authorization: string, secret: string, now: int64) =
        gate configuredTenant tenant hasHeader
            (fun a -> Auth.verifyReadAuthorization a secret tenant doc now)
            authorization

    let storageRead (configuredTenant: string, tenant: string, hasHeader: bool, authorization: string, secret: string, now: int64) =
        gate configuredTenant tenant hasHeader
            (fun a -> Auth.verifyStorageReadAuthorization a secret tenant now)
            authorization

    let storageWrite (configuredTenant: string, tenant: string, hasHeader: bool, authorization: string, secret: string, now: int64) =
        gate configuredTenant tenant hasHeader
    ...
    let mintToken (tenant: string, documentId: string, scopes: string[], userId: string, secret: string, now: int64, expiresIn: int64, jti: string) =
        Signet.mintToken tenant documentId (Signet.scopesFromStrings (List.ofArray scopes)) userId secret now expiresIn jti
...
module SiltBoundary =
    ...
    let objectResponse
        (baseUrl: string, tenant: string, kind: string, sha: string, body: string, recursive: bool,
    ...
    let commitHistoryResponse
        (baseUrl: string, tenant: string, sha: string, count: int, objects: IReadOnlyDictionary<string, string>)
        : byte[] =
    ...
    let treeChildShas (body: string) : string[] =
        ...
        | Some entries -> entries |> List.filter (fun e -> e.Kind = "tree") |> List.map (fun e -> e.Sha) |> Array.ofList
        | None -> [||]
