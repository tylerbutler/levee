# ADR-002: Client compatibility strategy for the Sluice migration

- **Status:** Accepted
- **Date:** 2026-06-30
- **Context:** Sluice-first migration — client package (`levee-driver`/`levee-client`) direction

## Context

The server-side migration is moving runtime ownership from Phoenix/Levee to
the standalone Gleam `sluice/` service, which speaks the same Socket.IO/REST
protocol as upstream Routerlicious (see
`client/packages/levee-driver/test/integration/sluice-contract.ts`, the
executable contract, and `sluice-routerlicious.test.ts`, the compatibility
suite that exercises an *unmodified* `@fluidframework/routerlicious-driver`
against Sluice). That work established the server-side principle: **Sluice is
...
Once Sluice implements the Routerlicious wire protocol directly, an
...
1. **Official Routerlicious-compatible clients become the primary long-term
   path.** Once Sluice passes the create/load/sync/reconnect/summaries/signals
   conformance surface in `sluice-routerlicious.test.ts` for a given target,
   consumers should use `@fluidframework/routerlicious-driver` (or
   `@fluidframework/routerlicious-urlResolver` /
   `@fluidframework/azure-client`-style wrappers) pointed at Sluice, not
...
3. **`@tylerbu/levee-driver`'s Phoenix Channels transport is legacy during the
   migration and becomes deprecated once Sluice conformance is reached.**
   ...
   `sluice-routerlicious.test.ts` passes for both `sluice-direct` and
...
4. **No new investment in the Phoenix Channels protocol.** Bug fixes and
   ...
   semantics) should be implemented against the Sluice/Routerlicious contract
   first, not the Phoenix Channels driver.

5. **`sluice-routerlicious.test.ts` is the acceptance suite of record** for
   deciding when the above transition is safe. The `test:sluice-routerlicious`
   pnpm script and `just test-sluice-routerlicious` recipe are the primary,
...
- Examples (`levee-example`, `levee-presence-tracker`) should migrate to the
  official Routerlicious/Sluice path once conformance holds, rather than
...
- No packages are renamed, removed, or marked deprecated in `package.json`
  ...
  the deprecation itself, since `sluice-routerlicious.test.ts` conformance
...
Rejected: it duplicates protocol logic that Sluice now implements natively
...
Rejected: `sluice-routerlicious.test.ts` still has live-gated and
`it.todo` gaps (see file header) for standalone Sluice vs. the Levee proxy
